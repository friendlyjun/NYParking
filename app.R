...
# setwd("~/NYC Data Science Academy/NYParkingDashboard/")
source("global.R")
# source("~/NYC Data Science Academy/NYParkingDashboard/global.R")
# source("~/NYC Data Science Academy/NYParkingDashboard/helpers.R")

# dbname = "~/NYC Data Science Academy/NYParkingDashboard/nyparking.sqlite"
# tblname = "nyparking"
# 
# dbname_map = "~/NYC Data Science Academy/NYParkingDashboard/Police_Precincts.sqlite"
...
server <- function(session,input, output) {
  ...
  output$fine_plot<- renderPlot(
    group_data() %>%
      ggplot(aes(x = get(input$item), y = total_fine)) +
      geom_col(fill = "#d95f0e",na.rm=TRUE) +
      xlab(input$item) +
      ylab("Total Fine Amount") +
      ggtitle("Total Fine($)")+
      theme_economist()
